...
const COLUMN_PREFS_KEY='auxilios.billing.excel.columns.v1';

function parts(v){
  if(!v)return{date:'',time:''};
  const p=new Intl.DateTimeFormat('es-AR',{timeZone:'America/Argentina/Buenos_Aires',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(new Date(v));
...
function parseAddress(value){
  const raw=text(value);if(!raw)return{street:'',locality:'',province:''};
  let p=raw.split(',').map(x=>x.trim()).filter(Boolean),province='',locality='',street='';
  if(p.length>1&&/(provincia|caba|ciudad autónoma|buenos aires|catamarca|chaco|chubut|córdoba|corrientes|entre ríos|formosa|jujuy|la pampa|la rioja|mendoza|misiones|neuquén|río negro|salta|san juan|san luis|santa cruz|santa fe|santiago del estero|tierra del fuego|tucumán)$/i.test(p[p.length-1]))province=normalizeProvince(p.pop());
  if(p.length>=2){locality=p.pop().replace(/^[A-Z]?\d{4}\s+/i,'').trim();street=p.join(', ');}else if(p.length===1){if(province)locality=p[0].replace(/^[A-Z]?\d{4}\s+/i,'').trim();else street=p[0];}
  if(!street&&!locality)street=raw;
...
function distanceAmounts(r){
  const rows=distanceComponents(r),total=sum(rows,'subtotal'),specific=rows.some(x=>x?.terrain==='asphalt'||x?.terrain==='gravel');
  if(specific)return{asphalt:sum(rows.filter(x=>x.terrain==='asphalt'),'subtotal'),gravel:sum(rows.filter(x=>x.terrain==='gravel'),'subtotal'),total};
  const asphalt=num(r.asphalt_km),gravel=num(r.gravel_km),km=asphalt+gravel;
  if(total<=0||km<=0)return{asphalt:0,gravel:0,total};
  const a=round2(total*asphalt/km);
  return{asphalt:a,gravel:round2(total-a),total};
}
...
function writeColumnPreferences(value){
  try{window.localStorage?.setItem(COLUMN_PREFS_KEY,JSON.stringify(value));}catch{}
}
function columnCatalog(type){return type==='toll'?tollColumns:serviceColumns;}
function defaultColumnIds(type){return type==='toll'?defaultTollIds:defaultServiceIds;}
function restoreColumnSelection(kind,type){
  const catalog=columnCatalog(type),valid=new Set(catalog.map(c=>c.id)),saved=readColumnPreferences()?.[kind]?.[type];
  if(!Array.isArray(saved))return new Set(defaultColumnIds(type));
  return new Set(saved.filter(id=>valid.has(id)));
}
function savePickerPreferences(){
  if(!picker)return;
  const prefs=readColumnPreferences(),scope={...(prefs[picker.kind]||{})};
  if(picker.serviceRows.length)scope.service=[...picker.serviceSelected];
  if(picker.tollRows.length)scope.toll=[...picker.tollSelected];
  prefs[picker.kind]=scope;
  writeColumnPreferences(prefs);
}
...
function openPicker(kind){
  try{
    const data=scopeData(kind);
    if(!data.serviceRows.length&&!data.tollRows.length)return notify('No hay registros para exportar con estos filtros','warning');
    picker={...data,serviceSelected:restoreColumnSelection(kind,'service'),tollSelected:restoreColumnSelection(kind,'toll'),busy:false};
    renderPicker();
...
function selectedColumns(type){if(!picker)return[];const catalog=columnCatalog(type),ids=type==='toll'?picker.tollSelected:picker.serviceSelected;return catalog.filter(c=>ids.has(c.id));}
function pickerGroup(type,title,columns,selected){return`<section class="obx-col-group" data-obx-group="${type}"><div class="obx-col-head"><div><b>${esc(title)}</b><small data-obx-count="${type}">${selected.size} de ${columns.length} columnas</small></div><div><button type="button" data-obx-select-all="${type}">Seleccionar Todos</button><button type="button" data-obx-deselect-all="${type}">Deseleccionar Todos</button></div></div><div class="obx-col-grid">${columns.map(c=>`<label><input type="checkbox" data-obx-col-type="${type}" data-obx-col="${esc(c.id)}" ${selected.has(c.id)?'checked':''}><span>${esc(c.header)}</span></label>`).join('')}</div></section>`;}
function syncPickerGroup(type){
  if(!picker)return;
  const set=type==='toll'?picker.tollSelected:picker.serviceSelected,columns=columnCatalog(type),back=document.getElementById('obx-picker-backdrop');
  back?.querySelectorAll(`[data-obx-col-type="${type}"]`).forEach(input=>{input.checked=set.has(input.dataset.obxCol);});
  const count=back?.querySelector(`[data-obx-count="${type}"]`);
  if(count)count.textContent=`${set.size} de ${columns.length} columnas`;
}
function renderPicker(){
  if(!picker)return;
  ensureStyle();
  let back=document.getElementById('obx-picker-backdrop');
  if(!back){
    back=document.createElement('div');back.id='obx-picker-backdrop';back.className='obx-picker-backdrop';document.body.appendChild(back);
    back.addEventListener('click',onPickerClick);back.addEventListener('change',onPickerChange);
  }
  const previousScroll=back.querySelector('.obx-picker-body')?.scrollTop||0;
  const svc=picker.serviceRows.length?pickerGroup('service','Columnas de servicios',serviceColumns,picker.serviceSelected):'',tolls=picker.tollRows.length?pickerGroup('toll','Columnas de peajes',tollColumns,picker.tollSelected):'';
  back.innerHTML=`<div class="obx-picker" role="dialog" aria-modal="true" aria-label="Elegir columnas para Excel"><header><div><h3>Exportar a Excel</h3><p>${esc(picker.label)} · Elegí qué columnas querés incluir. La selección usada al exportar queda guardada para la próxima vez.</p></div><button class="obx-close" type="button" data-obx-picker-close>×</button></header><div class="obx-picker-body">${svc}${tolls}</div><footer><div class="obx-picker-count">${picker.serviceRows.length?`${picker.serviceRows.length} servicios`:''}${picker.serviceRows.length&&picker.tollRows.length?' · ':''}${picker.tollRows.length?`${picker.tollRows.length} peajes`:''}</div><div><button class="obx-secondary" type="button" data-obx-picker-close>Cancelar</button><button class="obx-primary" type="button" data-obx-picker-export ${picker.busy?'disabled':''}>${picker.busy?'Preparando…':'Exportar Excel'}</button></div></footer></div>`;
  queueMicrotask(()=>{const body=back?.querySelector('.obx-picker-body');if(body)body.scrollTop=previousScroll;});
}
function onPickerChange(e){
  const el=e.target.closest('[data-obx-col]');if(!el||!picker)return;
  const type=el.dataset.obxColType,set=type==='toll'?picker.tollSelected:picker.serviceSelected;
  el.checked?set.add(el.dataset.obxCol):set.delete(el.dataset.obxCol);
  syncPickerGroup(type);
}
function onPickerClick(e){
  if(e.target===e.currentTarget||e.target.closest('[data-obx-picker-close]'))return closePicker();
  const all=e.target.closest('[data-obx-select-all]');
  if(all&&picker){const type=all.dataset.obxSelectAll,set=type==='toll'?picker.tollSelected:picker.serviceSelected;columnCatalog(type).forEach(c=>set.add(c.id));syncPickerGroup(type);return;}
  const none=e.target.closest('[data-obx-deselect-all]');
  if(none&&picker){const type=none.dataset.obxDeselectAll,set=type==='toll'?picker.tollSelected:picker.serviceSelected;set.clear();syncPickerGroup(type);return;}
  if(e.target.closest('[data-obx-picker-export]'))return confirmExport();
}
...
async function confirmExport(){
  ...
  const serviceCols=selectedColumns('service'),tollCols=selectedColumns('toll');
  if(picker.serviceRows.length&&!serviceCols.length&&picker.tollRows.length&&!tollCols.length)return notify('Seleccioná al menos una columna para exportar','warning');
  if(picker.serviceRows.length&&!picker.tollRows.length&&!serviceCols.length)return notify('Seleccioná al menos una columna de servicios','warning');
  if(picker.tollRows.length&&!picker.serviceRows.length&&!tollCols.length)return notify('Seleccioná al menos una columna de peajes','warning');
  picker.busy=true;renderPicker();
  try{
    const enriched=serviceCols.length?await fetchServiceRows(picker.serviceRows):[],sheets=[];
    if(enriched.length){
      if(picker.kind==='all'){
        const pending=enriched.filter(r=>r.billing_status==='pending'),reviewed=enriched.filter(r=>r.billing_status==='reviewed');
        if(pending.length)sheets.push({name:'Pendientes',columns:serviceCols,rows:pending});
        if(reviewed.length)sheets.push({name:'Revisados',columns:serviceCols,rows:reviewed});
      }else sheets.push({name:picker.label,columns:serviceCols,rows:enriched});
    }
    if(tollCols.length&&picker.tollRows.length)sheets.push({name:'Peajes',columns:tollCols,rows:picker.tollRows});
    if(!sheets.length)throw new Error('No hay columnas con datos para exportar.');
    excel().download({filename:fileBase(picker.S,picker.label),sheets});
    savePickerPreferences();
    const count=enriched.length+picker.tollRows.length;
    closePicker();notify(`${count} registros exportados a Excel`,'success');
  }catch(e){if(picker){picker.busy=false;renderPicker();}notify(e.message||'No se pudo exportar a Excel','error');}
}
...
function ensureStyle(){
  if(document.getElementById('auxilios-billing-excel-style'))return;
  const s=document.createElement('style');s.id='auxilios-billing-excel-style';
  s.textContent=`.obx-wrap{position:relative}.obx-trigger{height:34px;padding:0 12px;border-radius:8px;border:1px solid var(--border2);background:var(--card);color:var(--text);font:600 11px 'DM Sans',sans-serif;cursor:pointer;display:flex;align-items:center;gap:6px}.obx-trigger:hover{border-color:var(--amber);color:var(--amber)}.obx-menu{position:absolute;right:0;top:40px;width:250px;padding:6px;background:var(--card);border:1px solid var(--border2);border-radius:10px;box-shadow:0 12px 30px rgba(0,0,0,.45);z-index:500}.obx-menu button{width:100%;display:flex;flex-direction:column;align-items:flex-start;gap:2px;padding:10px;border:0;border-radius:7px;background:transparent;color:var(--text);font-family:'DM Sans',sans-serif;cursor:pointer;text-align:left}.obx-menu button:hover:not(:disabled){background:var(--bg)}.obx-menu button:disabled{opacity:.38;cursor:not-allowed}.obx-menu b{font-size:12px}.obx-menu small{font-size:10px;color:var(--muted2)}.obx-picker-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.72);display:flex;align-items:center;justify-content:center;padding:18px;z-index:12000}.obx-picker{width:min(1040px,97vw);height:min(780px,94vh);max-height:94vh;background:var(--card);border:1px solid var(--border2);border-radius:14px;box-shadow:0 24px 70px rgba(0,0,0,.55);display:flex;flex-direction:column;overflow:hidden}.obx-picker>header{flex:0 0 auto;display:flex;justify-content:space-between;gap:20px;align-items:flex-start;padding:16px 20px;border-bottom:1px solid var(--border)}.obx-picker h3{margin:0;font:24px 'Bebas Neue',sans-serif;letter-spacing:1px}.obx-picker p{margin:4px 0 0;color:var(--muted);font-size:12px}.obx-close{border:0;background:transparent;color:var(--muted);font-size:26px;cursor:pointer}.obx-picker-body{flex:1 1 auto;min-height:0;padding:16px 20px;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;scrollbar-gutter:stable;display:flex;flex-direction:column;gap:18px}.obx-col-group{flex:0 0 auto;border:1px solid var(--border);border-radius:10px;overflow:hidden}.obx-col-head{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px 14px;background:var(--bg)}.obx-col-head>div:first-child{display:flex;flex-direction:column;gap:2px}.obx-col-head b{font-size:12px}.obx-col-head small{font-size:10px;color:var(--muted)}.obx-col-head>div:last-child{display:flex;gap:6px;flex-wrap:wrap}.obx-col-head button,.obx-secondary,.obx-primary{border:1px solid var(--border2);border-radius:7px;padding:7px 10px;background:var(--card);color:var(--text);font:600 10px 'DM Sans',sans-serif;cursor:pointer}.obx-col-head button:hover,.obx-secondary:hover{border-color:var(--amber);color:var(--amber)}.obx-col-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0;padding:8px}.obx-col-grid label{min-width:0;display:flex;gap:8px;align-items:flex-start;padding:8px;border-radius:6px;font-size:11px;cursor:pointer}.obx-col-grid label:hover{background:var(--bg)}.obx-col-grid input{flex:0 0 auto;margin-top:1px;accent-color:var(--amber)}.obx-picker>footer{flex:0 0 auto;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 20px;border-top:1px solid var(--border);background:var(--bg)}.obx-picker>footer>div:last-child{display:flex;gap:8px}.obx-picker-count{font-size:11px;color:var(--muted)}.obx-primary{background:var(--amber);border-color:var(--amber);color:#111;padding:9px 14px}.obx-primary:disabled{opacity:.5;cursor:not-allowed}@media(max-width:780px){.obx-col-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.obx-col-head{align-items:flex-start;flex-direction:column}.obx-picker-backdrop{padding:8px}.obx-picker{width:100%;height:96vh;max-height:96vh}}@media(max-width:480px){.obx-col-grid{grid-template-columns:1fr}.obx-picker>header,.obx-picker-body,.obx-picker>footer{padding-left:12px;padding-right:12px}}`;
...
Object.assign(E,{exportCurrent,exportSelected,exportAllFiltered,serviceColumns,tollColumns,parseAddress,vehicleParts,distanceAmounts,restoreColumnSelection,syncVisibility});
