...
(() => {
  ...
  function inject() {
    if (!document.getElementById('company-billing-parameters-v4-css')) {
      document.head.insertAdjacentHTML('beforeend', `<style id="company-billing-parameters-v4-css">
        ...
        .bp4-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.bp4-grid.three{grid-template-columns:repeat(3,minmax(0,1fr))}.bp4-field{display:grid;gap:6px}.bp4-field>span{font-size:9px;font-weight:750;letter-spacing:.04em;text-transform:uppercase;color:var(--muted2)}.bp4-field.full{grid-column:1/-1}
  ...
  function close() {
    typeof closeModal === 'function' ? closeModal('modal-company-billing-v4') : document.getElementById('modal-company-billing-v4')?.classList.remove('open');
  }
  ...
  async function loadContext(companyId = null) {
    ...
    S.companyId = id;
    S.billing = billing.data || { setting: null, links: [], available_bases: [] };
    S.companyConfig = companyConfig.data || { services: [] };
    S.activeCard = rate.active;
    S.draftCard = rate.draft;
  ...
  function modalHtml() {
    ...
    const versionText = !S.rateCard
      ? '<b>Sin tarifario publicado.</b> Los parámetros generales se guardan igual; los recargos tarifarios necesitan una versión para quedar activos.'
      : S.draftCard
        ? `<b>Borrador v${esc(S.draftCard.version)}.</b> Los cambios de recargos quedan dentro de esa misma versión pendiente.`
        : `<b>Tarifario publicado v${esc(S.activeCard?.version)}.</b> Si cambiás recargos, AuxiliOS crea una versión nueva para preservar el histórico.`;
  ...
  function renderTollHelp() {
    ...
    const panel = document.getElementById('bp4-toll-help');
    if (!panel) return;
    if (mode === 'manual') {
      panel.className = 'bp4-help full manual';
      panel.innerHTML = '<b>Carga real / comprobante</b>El importe real se registra durante el servicio y se factura con ese valor.';
    } else if (mode === 'not_applicable') {
      panel.className = 'bp4-help full off';
      panel.innerHTML = '<b>No corresponde</b>AuxiliOS no solicita ni incorpora peajes para esta prestadora.';
    } else {
      panel.className = 'bp4-help full route';
      panel.innerHTML = '<b>Estimación automática por ruta</b>AuxiliOS usa el recorrido calculado como referencia para estimar los peajes.';
    }
  }
  ...
  function updateAmountLabel(prefix) {
    const label = document.getElementById(`bp4-${prefix}-label`);
    if (label) label.textContent = document.getElementById(`bp4-${prefix}-mode`)?.value === 'fixed' ? 'Monto fijo' : 'Porcentaje (%)';
  }
  ...
  async function openParameters(companyId = null) {
    ...
    S.companyId = id;
    const body = document.getElementById('bp4-modal-body');
    if (body) body.innerHTML = '<div class="bp4-help">Cargando parámetros…</div>';
  ...
  function setError(message = '') {
    const el = document.getElementById('bp4-error');
    if (!el) return;
  ...
  function collectRule(type) {
    ...
    return {
      type,
      enabled: Boolean(document.getElementById(`bp4-${prefix}-enabled`)?.checked),
      calculation_mode: document.getElementById(`bp4-${prefix}-mode`)?.value || 'percentage',
      amount: Math.max(0, Number(document.getElementById(`bp4-${prefix}-amount`)?.value || 0)),
  ...
  async function saveRule(cardId, model) {
    ...
    const removed = await _db.from('company_rate_rule_exceptions').delete().eq('rule_id', saved.data.rule_id);
    if (removed.error) throw removed.error;
    if (model.exceptions.length) {
      const inserted = await _db.from('company_rate_rule_exceptions').insert(model.exceptions.map(conceptId => ({ rate_card_id: cardId, rule_id: saved.data.rule_id, concept_id: conceptId })));
  ...
  async function saveParameters() {
    ...
    const from = document.getElementById('bp4-from')?.value || today();
    const until = document.getElementById('bp4-until')?.value || null;
    ...
    const validation = validateRules(models);
    if (validation) return setError(validation);

    const button = document.getElementById('bp4-save');
    S.busy = true; setError('');
    if (button) { button.disabled = true; button.textContent = 'Guardando…'; }
    try {
      const setting = S.billing?.setting || {};
      const billingPayload = {
        billing_setting_id: setting.billing_setting_id || null,
        company_id: S.companyId,
        contract_id: setting.contract_id || null,
        route_mode: document.getElementById('bp4-route')?.value || 'base_origin_destination_base',
        toll_calculation_mode: document.getElementById('bp4-tolls')?.value || 'route_estimate',
        valid_from: from,
        valid_until: until,
        requires_verified_base: Boolean(setting.requires_verified_base),
        is_active: active,
      ...
      const billing = await _db.rpc('save_company_billing_configuration', { p_payload: billingPayload });
      if (billing.error) throw billing.error;
      ...
      if (hasConfiguredRule || S.rateCard) {
        ...
        const draftId = draft.data?.rate_card_id;
        if (!draftId) throw new Error('No se pudo preparar la versión tarifaria para los recargos.');
  ...
  Object.assign(window, {
    abrirConfiguracionFacturacionEmpresa: companyId => openParameters(companyId),
