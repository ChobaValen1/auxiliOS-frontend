...
(() => {
  ...
  function closeJourneyModal(){
    ...
    const root=mountModal({eyebrow:`Jornada #${log.log_id}`,title:'Cerrar jornada desde Administración',wide:true,
      ...
      footer:`<button class="btn btn-ghost" type="button" data-jat-close>Cancelar</button><button class="btn btn-primary" type="submit" form="jat-close-form">Cerrar jornada</button>`});
  ...
  function installFooterActions(){
    ...
    actions.innerHTML=`${isAdmin()?'<button class="btn btn-ghost" type="button" data-jat-edit>✎ Editar jornada</button>':''}${canClose?'<button class="btn btn-primary" type="button" data-jat-close-journey>■ Cerrar jornada</button>':''}<button class="btn btn-ghost" type="button" data-jat-history>Historial</button>${isAdmin()?'<button class="btn jat-btn-danger" type="button" data-jat-void>Anular jornada</button>':''}`;
