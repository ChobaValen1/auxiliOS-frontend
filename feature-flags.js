...
async function activate(flags){
 // El workspace de Nuevo Servicio es canónico y se carga una sola vez desde config.js.
 // Feature flags conserva únicamente módulos realmente opcionales.
 ...
 if(flags.operator_console_v2||flags.service_editing_tolls_v1){
...
async function refresh(){
 const client=db();if(!client||loading)return;loading=true;
 try{
  const {data:{session},error:sessionError}=await client.auth.getSession();if(sessionError)throw sessionError;
  const userId=session?.user?.id||null;
  if(!userId){STATE.flags={service_workspace_v2:true};STATE.userId=null;STATE.ready=true;loadedUser=null;window.dispatchEvent(new CustomEvent('auxilios:features-ready',{detail:STATE}));return;}
  if(loadedUser===userId&&STATE.ready)return;
  const {data,error}=await client.from('user_feature_flags').select('feature_key,enabled').eq('enabled',true);if(error)throw error;
  const flags=Object.fromEntries((data||[]).map(row=>[row.feature_key,Boolean(row.enabled)]));
  // El workspace actual ya no es experimental: esta marca queda solo para módulos que consultan estado.
 ...
 }catch(error){console.warn('[Feature flags]',error.message||error);STATE.ready=true;}
