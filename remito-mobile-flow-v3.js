...
function customerStep(step){
  ...
  $('#rem-telefono-no-informa',step)?.addEventListener('change',e=>{const input=$('#rem-telefono');if(!input)return;if(e.target.checked){input.value='';input.classList.remove('rem-field-error');$('#err-telefono')?.classList.remove('visible')}input.disabled=e.target.checked||$('[data-remito-field="customer_phone"]',step)?.dataset.mode==='hidden'});
...
/* Teléfono: 10 dígitos (código de área sin 0 + número sin 15), para poder
   enviarle el remito por WhatsApp. Obligatorio salvo "El cliente no informa teléfono". */
const phoneValid=v=>/^\d{10}$/.test(String(v||'').trim());
function validateCustomerFields(){let ok=true;for(const [key,id,errorId] of [['customer_document','rem-cuit','err-documento'],['customer_phone','rem-telefono','err-telefono']]){const row=document.querySelector(`[data-remito-field="${key}"]`),input=document.getElementById(id),error=document.getElementById(errorId),value=String(input?.value||'').trim(),noInforma=key==='customer_phone'&&!!document.getElementById('rem-telefono-no-informa')?.checked;let missing=row?.dataset.mode==='required'&&!value&&!noInforma;const invalid=key==='customer_phone'&&row?.dataset.mode!=='hidden'&&!!value&&!phoneValid(value);if(error&&key==='customer_phone')error.textContent=invalid?'Tiene que tener 10 números: código de área sin 0 y número sin 15':'Cargá el teléfono o marcá que el cliente no lo informa';missing=missing||invalid;input?.classList.toggle('rem-field-error',missing);error?.classList.toggle('visible',missing);if(missing)ok=false}return ok}
...
async function selectMapLocation(kind,index){
  ...
  const seq=++state.seq;if(state.timer)clearTimeout(state.timer);state.timer=null;state.loading=true;renderMapSuggestions(kind);try{const {data,error}=await client.functions.invoke('maps-proxy',{body:{action:'place',placeId:suggestion.placeId,sessionToken:state.token||newMapToken()}});if(seq!==state.seq)return;if(error)throw error;const formatted=data?.formattedAddress||suggestion.text||suggestion.mainText||'';state.place={place_id:data?.placeId||suggestion.placeId,latitude:data?.location?.latitude??null,longitude:data?.location?.longitude??null,formatted_address:formatted};const input=document.getElementById(`rem-${mapKey[kind]}`);if(input)input.value=formatted;state.token=newMapToken();state.suggestions=[];state.error='';await computeRouteDistance();}catch(error){if(seq!==state.seq)return;state.place=null;clearRouteDistance();state.error=error?.message||'No se pudo validar la dirección'}finally{if(seq===state.seq){state.loading=false;renderMapSuggestions(kind)}}}
...
function resetMapLocations(){clearRouteDistance();for(const [kind,state] of Object.entries(mapLocations)){if(state.timer)clearTimeout(state.timer);Object.assign(state,{token:'',timer:null,seq:state.seq+1,suggestions:[],place:null,loading:false,error:''});document.getElementById(`rem-${mapKey[kind]}`)?.classList.remove('rem-field-error');renderMapSuggestions(kind)}}
...
function setAdHocMode(enabled){
  ...
  if(next&&!adHocMode){
    ...
    attach('rem-nro-prestadora','order');attach('rem-patente','plate');attach('rem-marca-modelo','vehicle');attach('rem-origen','origin');attach('rem-destino','destination');attach('rem-km','km');const km=document.getElementById('rem-km');if(km){km.readOnly=true;km.setAttribute('aria-readonly','true')}
