...
(() => {
  ...
  async function open(invoiceId = null) {
    ...
    if (!screen) return;
    window.dispatchEvent(new CustomEvent('auxilios:navigation-changed', { detail: { screen: 'facturas' } }));
  ...
  function filterOptions() {
    const companies = [
      ...
      ...S.filters.companies.map(item => `<option value="${esc(item.company_id)}" ${String(S.company) === String(item.company_id) ? 'selected' : ''}>${esc(item.company_name)}</option>`)
    ].join('');
    const periods = [
      '<option value="">Todos los períodos</option>',
      ...S.filters.periods.map(period => `<option value="${esc(period)}" ${S.period === period ? 'selected' : ''}>${esc(periodLabel(period))}</option>`)
    ].join('');
  ...
  const creditTypeLabel = value => value === 'NCA' ? 'Nota de Crédito A' : value === 'NCB' ? 'Nota de Crédito B' : value === 'NCC' ? 'Nota de Crédito C' : 'Nota de Crédito';
  const creditNumber = note => note ? `${creditTypeLabel(note.document_type)} ${note.point_of_sale || ''}-${note.document_number || ''}`.trim() : '';
  ...
  function actionMenuMarkup(row) {
    ...
    const pdfActions = row.pdf_path
      ? `<button type="button" data-oi-row-action="view-pdf" data-invoice-id="${esc(row.invoice_id)}">Ver PDF</button><button type="button" data-oi-row-action="attach-pdf" data-invoice-id="${esc(row.invoice_id)}">Reemplazar PDF</button>`
    ...
    const lifecycleActions = canManage() && active
      ? `<button type="button" data-oi-row-action="credit-note" data-invoice-id="${esc(row.invoice_id)}">Emitir Nota de Crédito</button><button type="button" class="danger" data-oi-row-action="annul" data-invoice-id="${esc(row.invoice_id)}">Anular</button>`
      : '';
    return `${pdfActions}<button type="button" data-oi-row-action="excel" data-invoice-id="${esc(row.invoice_id)}">Descargar Excel</button>${lifecycleActions}`;
  }
  ...
  function toggleActionMenu(trigger, id) {
    ...
    const row = rowById(id) || (String(S.detail?.invoice?.invoice_id) === String(id) ? S.detail.invoice : null);
    if (!row) return;
    closeActionMenu();
  ...
  function detailMarkup() {
    ...
    const tollLines = Array.isArray(detail.toll_lines) ? detail.toll_lines : [];
    const creditNote = detail.credit_note || null;
    const creditSection = creditNote
      ? `<section class="oi-section oi-credit"><h4>Nota de Crédito</h4><div class="oi-grid"><div><small>Comprobante</small><b>${esc(creditNumber(creditNote))}</b></div><div><small>Fecha</small><b>${esc(invoiceDate(creditNote.issued_on, creditNote.created_at))}</b></div><div><small>Importe</small><b>${esc(money(creditNote.amount, creditNote.currency))}</b></div><div><small>Alcance</small><b>Total</b></div></div></section>`
      : '';
  ...
  function conceptCountText(row) {
    return `${num(row.service_count)} servicios y ${num(row.toll_count)} peajes`;
  }
  ...
  function actionModalMarkup() {
    ...
    if (action.type === 'credit-note') {
      ...
      return `<section class="oi-modal" role="dialog" aria-modal="true"><header><div><small>${esc(row.invoice_number || 'Factura')}</small><h3>Emitir Nota de Crédito</h3></div><button class="oi-button" data-oi="close-action" ${busy ? 'disabled' : ''}>×</button></header>
        ...
        <footer><button class="oi-button" data-oi="close-action" ${busy ? 'disabled' : ''}>Cancelar</button><button class="oi-button primary" data-oi="confirm-credit-note" ${busy ? 'disabled' : ''}>${busy ? 'Emitiendo…' : 'Emitir Nota de Crédito'}</button></footer></section>`;
    }
    return '';
  }
  ...
  function openCreditNote(id) {
    ...
    S.action = {
      type: 'credit-note', invoiceId: id,
      form: { document_type: type, point_of_sale: row.point_of_sale || '', document_number: '', issued_on: todayLocalDate(), notes: '' }
    };
    render();
  }
  ...
  async function confirmAnnul() {
    ...
    render();
    try {
      const { data, error } = await db().rpc('annul_operator_invoice_v2', { p_invoice_id: action.invoiceId, p_reason: reason });
      if (error) throw error;
      S.action = null;
      S.detail = null;
      notify(`Factura anulada · ${num(data?.released_service_count)} servicios y ${num(data?.released_toll_count)} peajes volvieron a Facturación`, 'success');
      await load();
    } catch (error) {
  ...
  function choosePdf(id) {
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        ...
        const { error: uploadError } = await storage.upload(path, file, { upsert: true, contentType: 'application/pdf', cacheControl: '3600' });
  ...
  async function exportExcel(id) {
    try {
      const detail = String(S.detail?.invoice?.invoice_id) === String(id) ? S.detail : await fetchDetail(id);
      const invoice = detail?.invoice || {};
      const lines = Array.isArray(detail?.lines) ? detail.lines : [];
      const tollLines = Array.isArray(detail?.toll_lines) ? detail.toll_lines : [];
      ...
      const summaryRows = [
        ...
        { label: 'Peajes', value: String(invoice.toll_count || tollLines.length) },
      ...
      const sheets = [{ name: 'Resumen', columns: summaryColumns, rows: summaryRows }];
      if (lines.length) {
        const serviceColumns = [
          ...
          { header: 'Estado vínculo', width: 18, key: 'link_status' }
        ];
        const serviceRows = lines.map(line => {
          ...
          return {
            ...
            customer: service.customer_name || '', vehicle: service.vehicle_make_model || '', plate: service.vehicle_plate || '',
            origin: service.origin || '', destination: service.destination || '', amount: num(line.company_amount),
            currency: line.currency || invoice.currency || 'ARS',
            link_status: line.released_at ? 'Liberado por anulación' : 'Facturado'
          };
        });
        sheets.push({ name: 'Servicios', columns: serviceColumns, rows: serviceRows });
      }
      ...
      if (tollLines.length) {
        const tollColumns = [
          ...
          { header: 'Ruta', width: 22, key: 'road' },
          { header: 'Sentido', width: 18, key: 'direction' },
          { header: 'Cantidad', width: 10, key: 'quantity', type: 'number' },
          { header: 'Importe', width: 16, key: 'amount', type: 'number' },
          { header: 'Moneda', width: 10, key: 'currency' },
          { header: 'Estado vínculo', width: 18, key: 'link_status' }
        ];
        const tollRows = tollLines.map(line => {
          const toll = line.toll_snapshot || {};
          const service = line.service_snapshot || {};
          return {
            line_number: line.line_number,
            order: service.service_order_number || '', service_number: service.service_number || '',
            scheduled_for: invoiceDate(String(toll.crossed_at || service.scheduled_for || '').slice(0, 10), toll.crossed_at || service.scheduled_for),
            toll_name: toll.toll_name || 'Peaje', road: toll.road || '', direction: toll.direction || '',
            quantity: num(toll.quantity || 1), amount: num(line.amount), currency: line.currency || invoice.currency || 'ARS',
            link_status: line.released_at ? 'Liberado por anulación' : 'Facturado'
          };
        });
  ...
  function onInput(event) {
    ...
    if (field && S.action) {
      ...
      } else S.action.form[field] = event.target.value;
      return;
    }
  ...
  function onChange(event) {
    const field = event.target.dataset?.oiActionField;
    if (field && S.action) { S.action.form[field] = event.target.value; return; }
    if (event.target.id === 'oi-company-filter') { S.company = event.target.value || ''; return load(); }
    if (event.target.id === 'oi-period-filter') { S.period = event.target.value || ''; return load(); }
  }

  function onClick(event) {
    const menu = event.target.closest('[data-oi-menu]');
    if (menu) { event.stopPropagation(); return toggleActionMenu(menu, menu.dataset.oiMenu); }
  ...
  function onDocumentClick(event) {
    ...
    if (!event.target.closest('[data-oi-menu]') && !event.target.closest('#oi-action-menu')) closeActionMenu();
  }
