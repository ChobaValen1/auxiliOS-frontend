-- AuxiliOS · resumen operativo seguro para la tarjeta activa del Chofer.
-- El kilometraje expuesto es exclusivamente Origen → Destino y proviene del
-- cálculo ya persistido por Google Routes. Los campos nuevos no exponen importes
-- ni amplían los datos personales que requiere el prellenado existente del remito.
...
begin
  ...
  from (
    select s.scheduled_for,s.created_at,jsonb_build_object(
      ...
      'has_excesses',(
        exists(
          select 1 from public.operator_service_excess_charges e
          where e.service_id=s.service_id
        )
        or exists(
          select 1 from public.remito_excess_reports x
          where x.remito_id=s.remito_id
        )
      ),
    ...
    left join lateral (
      ...
      select case
        when toll_count=0 then 'Sin peajes previstos'
        when provider_only then 'A cargo de la prestadora'
        when customer_only then 'A cargo del cliente'||case
          when customer_methods is null then '' else ' · '||customer_methods end
        else 'Pago mixto'||case
          when customer_methods is null then '' else ' · Cliente: '||customer_methods end
      end payment_summary
      from payment_data
...
comment on function public.get_driver_operator_queue_v2() is
  'Cola privada del Chofer con resumen operativo sin importes; el KM es sólo Origen-Destino calculado por Google Routes.';
