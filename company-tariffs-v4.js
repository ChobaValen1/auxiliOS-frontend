...
(() => {
  ...
  function injectStyles() {
    ...
    document.head.insertAdjacentHTML('beforeend', `<style id="company-tariffs-v4-css">
      .ct4{display:grid;gap:10px;min-height:0}.ct4-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.ct4-head h2{margin:0}.ct4-head p{margin:4px 0 0;font-size:10px;color:var(--muted2)}
      .ct4-toolbar{display:flex;align-items:end;gap:10px;flex-wrap:wrap;padding:9px 11px;border:1px solid var(--border);border-radius:10px;background:var(--panel)}.ct4-field{display:grid;gap:5px}.ct4-field>span{font-size:8px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--muted2)}.ct4-company-field{width:min(340px,100%)}
      .ct4-stats{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-left:auto}.ct4-stat{display:inline-flex;gap:4px;align-items:center;padding:5px 8px;border:1px solid var(--border);border-radius:999px;background:var(--bg);font-size:8px;color:var(--muted2)}.ct4-stat b{font-size:10px;color:var(--text)}.ct4-stat.pending{color:var(--amber);border-color:rgba(245,166,35,.28)}
      .ct4-panel{border:1px solid var(--border);border-radius:10px;background:var(--panel);overflow:hidden;min-height:0}.ct4-panel-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 11px;border-bottom:1px solid var(--border)}.ct4-panel-head h3{margin:0;font-size:11px}.ct4-panel-head p{margin:0;font-size:8px;color:var(--muted2)}
      .ct4-table-wrap{overflow:auto;max-height:calc(100vh - 255px)}.ct4-table{width:100%;border-collapse:collapse}.ct4-table th,.ct4-table td{padding:9px 10px;border-bottom:1px solid var(--border);text-align:left;vertical-align:top;font-size:9px}.ct4-table th{position:sticky;top:0;z-index:1;background:var(--panel);font-size:7.5px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted)}.ct4-table strong{display:block;font-size:10px;color:var(--text)}.ct4-table small{display:block;margin-top:2px;font-size:8px;line-height:1.35;color:var(--muted2)}.ct4-table tr:last-child td{border-bottom:0}
      .ct4-chip{display:inline-flex;align-items:center;padding:3px 6px;border:1px solid var(--border2);border-radius:999px;font-size:7.5px;color:var(--muted2);white-space:nowrap}.ct4-chip.pending{color:var(--amber);border-color:rgba(245,166,35,.3)}.ct4-chip.future{color:var(--primary);border-color:rgba(79,142,247,.3)}
      .ct4-price-main{font-weight:850;color:var(--text);white-space:nowrap}.ct4-price-km{margin-top:2px;font-size:8px;color:var(--muted2)}.ct4-current-date{margin-top:3px;font-size:7.5px;color:var(--muted)}
      .ct4-next{display:grid;gap:3px;min-width:150px}.ct4-next-row{padding:5px 7px;border:1px solid rgba(79,142,247,.22);border-radius:7px;background:rgba(79,142,247,.04)}.ct4-next-row b{font-size:8px;color:var(--primary)}.ct4-next-row span{display:block;margin-top:2px;font-size:8px;color:var(--text)}
      .ct4-actions{display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end}.ct4-action{border:1px solid var(--border2);background:var(--bg);color:var(--text);border-radius:6px;padding:5px 7px;font-size:8px;cursor:pointer}.ct4-action.primary{border-color:rgba(79,142,247,.35);color:var(--primary)}.ct4-action.danger{border-color:rgba(226,80,74,.3);color:var(--red)}
      .ct4-exceptions{display:grid;gap:3px;min-width:145px}.ct4-exception{display:flex;justify-content:space-between;gap:6px;align-items:center}.ct4-exception span{font-size:7.5px;color:var(--muted2)}.ct4-exception b{font-size:8px;color:var(--text)}
      .ct4-empty{padding:24px;text-align:center;color:var(--muted2);font-size:10px}.ct4-error{padding:10px 12px;border:1px solid rgba(226,80,74,.3);border-radius:8px;background:rgba(226,80,74,.06);color:var(--red);font-size:9px}
      ...
      .ct4-embedded>.ct4-head,.ct4-embedded .ct4-company-field{display:none}.ct4-embedded .ct4-toolbar{padding:6px 0;border:0;background:transparent}.ct4-embedded .ct4-stats{margin-left:0}
      @media(max-width:900px){.ct4-table th:nth-child(2),.ct4-table td:nth-child(2){display:none}.ct4-table-wrap{max-height:none}}
      @media(max-width:650px){.ct4-grid,.ct4-history-row,.ct4-schedule-item{grid-template-columns:1fr}.ct4-stats{margin-left:0}.ct4-actions{justify-content:flex-start}.ct4-table{min-width:760px}}
    </style>`);
  }
  ...
  function shell(instance){
    ...
    instance.root.innerHTML=`<div class="ct4 ${embedded?'ct4-embedded':''}">
      <div class="ct4-head"><div><h2>Tarifas</h2><p>Precios por servicio. Podés editar el valor vigente o programar una nueva vigencia futura.</p></div></div>
  ...
  function priceChanges(instance,service,baseId=null){
    let previous=currentForBase(service,baseId);const changes=[];
  ...
  function allScheduleChanges(instance,service){
    ...
    const rows=[];for(const key of keys)rows.push(...priceChanges(instance,service,key||null));
  ...
  function nextPriceHtml(instance,service){
    ...
    return`<div class="ct4-next"><div class="ct4-next-row"><b>Desde ${dateLabel(next.valid_from)}</b><span>${formatPrice(instance,service,next)}</span></div>${changes.length>1?`<button class="ct4-action" type="button" data-ct4-schedules="${esc(service.concept_id)}">Ver ${changes.length} vigencias</button>`:''}</div>`;
  }
  ...
  function exceptionsHtml(instance,service){
    ...
    return`<div class="ct4-exceptions">${current.slice(0,2).map(row=>`<div class="ct4-exception"><span>${esc(row.base_name||'Base')}</span><b>${formatPrice(instance,service,row)}</b></div>`).join('')}${current.length>2?`<span class="ct4-chip">+${current.length-2} bases</span>`:''}${scheduled.length?`<button class="ct4-action" type="button" data-ct4-schedules="${esc(service.concept_id)}">${scheduled.length} cambio${scheduled.length===1?'':'s'} programado${scheduled.length===1?'':'s'}</button>`:''}</div>`;
  }

  function renderInstance(instance){
    const content=instance.root.querySelector('[data-ct4-content]'),error=instance.root.querySelector('[data-ct4-error]'),stats=instance.root.querySelector('[data-ct4-stats]');
    if(!content)return;if(error)error.innerHTML='';
    ...
    const d=instance.data,enabled=Number(d.enabled_count||0),priced=Number(d.priced_count||0),pending=Math.max(enabled-priced,0);
    const scheduledServices=new Set((instance.schedule||[]).map(x=>String(x.concept_id))).size;
    if(stats)stats.innerHTML=`<span class="ct4-stat"><b>${enabled}</b> habilitados</span><span class="ct4-stat"><b>${priced}</b> con precio</span><span class="ct4-stat ${pending?'pending':''}"><b>${pending}</b> sin precio</span>${scheduledServices?`<span class="ct4-stat"><b>${scheduledServices}</b> con vigencias futuras</span>`:''}`;
  ...
  async function loadInstance(instance){
    ...
    const [prices,schedule]=await Promise.all([_db.rpc('get_company_service_prices_v1',{p_company_id:instance.companyId}),_db.rpc('get_company_service_price_schedule_v1',{p_company_id:instance.companyId})]);
    ...
    instance.data=prices.data||{services:[],bases:[]};instance.schedule=Array.isArray(schedule.data)?schedule.data:[];renderInstance(instance);
  }
  ...
  function openPriceEditor(instance,conceptId,{baseId=null,selectingBase=false,validFrom=today(),programming=false,scheduled=false}={}){
    ...
    const dateField=`<label class="ct4-field ct4-full"><span>Vigente desde *</span><input class="form-input" type="date" id="ct4-valid-from" min="${today()}" value="${esc(validFrom)}" ${scheduled?'disabled':''}></label><div class="ct4-note ct4-full">${scheduled?'Esta vigencia ya está programada. Para cambiar la fecha, cancelala y creá una nueva.':'Hoy aplica inmediatamente. Una fecha futura queda programada y se activa automáticamente para los servicios de esa fecha en adelante.'}</div>`;
  ...
  async function savePrice(){
    const editor=activeEditor;if(!editor||!canWrite())return;const instance=instances.get(editor.instanceId);if(!instance)return;
    const validFrom=document.getElementById('ct4-valid-from')?.value||today();if(validFrom<today())return showModalError('La vigencia no puede comenzar en una fecha pasada.');
    const baseId=editor.selectingBase?document.getElementById('ct4-rate-base')?.value||null:editor.baseId;if(editor.selectingBase&&!baseId)return showModalError('Seleccioná una base.');
    const payload={company_id:instance.companyId,concept_id:editor.conceptId,billing_base_id:baseId};
    if(editor.service.distance_chargeable){const movement=Number(document.getElementById('ct4-movement')?.value),km=Number(document.getElementById('ct4-km')?.value);if(!Number.isFinite(movement)||movement<0||!Number.isFinite(km)||km<0)return showModalError('Completá valores válidos para movida y kilómetro.');payload.movement_price=movement;payload.km_price=km;}else{const value=Number(document.getElementById('ct4-unit')?.value);if(!Number.isFinite(value)||value<0)return showModalError('Ingresá un valor válido.');payload.unit_price=value;}
    const future=validFrom>today();if(future)payload.valid_from=validFrom;
    const button=document.getElementById('ct4-rate-save');if(button){button.disabled=true;button.textContent=future?'Programando…':'Guardando…';}
    ...
    close('modal-ct4-rate');activeEditor=null;notify(future?`Precio programado desde ${dateLabel(validFrom)}`:'Precio actualizado','success');await reloadCompany(instance.companyId);
  }
  ...
  async function cancelSchedule(instance,service,row){
    ...
    close('modal-ct4-history');notify('Vigencia programada cancelada','success');await reloadCompany(instance.companyId);
  }

  function scheduleItemHtml(instance,service,row){const scope=row.billing_base_id?row.base_name||'Base':'Precio general';return`<div class="ct4-schedule-item"><div><b>${dateLabel(row.valid_from)}</b><small>${esc(scope)}</small></div><div>${formatPrice(instance,service,row)}</div><div class="ct4-actions">${canWrite()?`<button class="ct4-action primary" type="button" data-ct4-edit-schedule data-date="${esc(row.valid_from)}" data-base="${esc(row.billing_base_id||'')}">Editar</button><button class="ct4-action danger" type="button" data-ct4-cancel-schedule data-date="${esc(row.valid_from)}" data-base="${esc(row.billing_base_id||'')}">Cancelar</button>`:''}</div></div>`;}

  function openSchedules(instance,conceptId){
    const service=serviceFor(instance,conceptId);if(!service)return;const changes=allScheduleChanges(instance,service);
    document.getElementById('ct4-history-title').textContent=`Vigencias programadas · ${service.name}`;
    const body=document.getElementById('ct4-history-body');body.innerHTML=changes.length?`<div class="ct4-schedule-list">${changes.map(row=>scheduleItemHtml(instance,service,row)).join('')}</div>`:'<div class="ct4-empty">No hay cambios futuros programados.</div>';
  ...
  function historyValue(instance,service,row){if(!row)return'—';if(service.distance_chargeable)return`${money(row.movement_price,instance.data?.currency)} movida · ${money(row.km_price,instance.data?.currency)}/km`;return money(row.unit_price,instance.data?.currency);}

  async function openHistory(instance,conceptId){
    const service=serviceFor(instance,conceptId);if(!service)return;document.getElementById('ct4-history-title').textContent=`Historial · ${service.name}`;document.getElementById('ct4-history-body').innerHTML='<div class="ct4-empty">Cargando historial…</div>';open('modal-ct4-history');
    const result=await _db.rpc('get_company_service_price_history_v1',{p_company_id:instance.companyId,p_concept_id:conceptId});if(result.error){document.getElementById('ct4-history-body').innerHTML=`<div class="ct4-error">${esc(result.error.message||'No se pudo cargar el historial.')}</div>`;return;}
    const rows=Array.isArray(result.data)?result.data:[];document.getElementById('ct4-history-body').innerHTML=rows.length?`<div class="ct4-history">${rows.map(row=>`<div class="ct4-history-row"><div><b>${esc(row.actor_name||'Usuario')}</b><span>${row.occurred_at?new Date(row.occurred_at).toLocaleString('es-AR'):'—'}</span></div><div><b>${esc(row.base_name||'Precio general')}</b><span>${esc(row.operation==='INSERT'?'CREACIÓN':row.operation==='DELETE'?'ELIMINACIÓN':'MODIFICACIÓN')}</span></div><div><b>${historyValue(instance,service,row.before)} → ${historyValue(instance,service,row.after)}</b><span>Cambio registrado automáticamente.</span></div></div>`).join('')}</div>`:'<div class="ct4-empty">Todavía no hay cambios registrados para este precio.</div>';
  }

  async function reloadCompany(companyId){const targets=[...instances.values()].filter(i=>String(i.companyId)===String(companyId));await Promise.all(targets.map(loadInstance));window.AuxiliosEmpresasV2?.refresh?.(companyId);}

  async function mount(root,{mode='embedded',companyId='',id=null}={}){if(!root)return null;injectStyles();ensureModals();const instanceId=id||root.id||`ct4-${Math.random().toString(36).slice(2)}`;const instance={id:instanceId,root,mode,companyId:companyId||'',companies:[],data:null,schedule:[],loading:false};instances.set(instanceId,instance);shell(instance);if(mode==='standalone')await loadCompanies(instance);if(instance.companyId)await loadInstance(instance);else renderInstance(instance);return instance;}
