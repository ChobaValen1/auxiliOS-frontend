(function(){
  ...
  async function embed(serviceId,hostId='osv4-review-slot'){
    const changed=E.serviceId!==serviceId;E.serviceId=serviceId;E.hostId=hostId;if(changed){E.decisions=new Map();E.note=''}E.busy=false;const host=document.getElementById(hostId);if(!host)return;if(!canResolve()||embeddedReadOnly()){E.detail=null;host.innerHTML='<div class="os-embedded-ok">Documento disponible en modo lectura.</div>';return}host.innerHTML='<div class="os-review-loading">Cargando diferencias…</div>';
    const {data,error}=await _db.rpc('get_operator_service_remito_review_v3',{p_service_id:serviceId});if(error){host.innerHTML=`<div class="os-review-errors visible">${esc(error.message)}</div>`;return}E.detail=data;preservePlannedRows();if(changed){const draft=await _db.rpc('get_operator_service_review_draft_v1',{p_service_id:serviceId});if(!draft.error&&draft.data?.decisions){E.decisions=new Map(Object.entries(draft.data.decisions));E.note=String(draft.data.note||'')}}const keys=new Set([...(data.original_reported?.tolls||data.reported?.tolls||[]).map(r=>'toll:'+r.toll_report_id),...(data.original_reported?.excesses||data.reported?.excesses||[]).map(r=>'excess:'+r.excess_report_id)]);for(const key of E.decisions.keys())if(!keys.has(key))E.decisions.delete(key);renderEmbedded();
  }
  function decideEmbedded(kind,id,value){if(E.busy)return;const w=window.OperatorServices?.S?.wizard;if(w?.mode==='view')return;const key=kind+':'+id,current=E.decisions.get(key)||{};E.decisions.set(key,{value,reason:value==='rejected'?(current.reason||''):''});const field=kind==='toll'?'tolls':'excess_charges',reportKey=kind==='toll'?'toll_report_id':'excess_report_id',rows=w?.data?.commercial_addons?.[field],source=(E.detail?.original_reported?.[kind==='toll'?'tolls':'excesses']||E.detail?.reported?.[kind==='toll'?'tolls':'excesses']||[]).find(r=>String(r[reportKey])===String(id));if(rows&&source){let index=rows.findIndex(r=>String(r[reportKey])===String(id));if(index<0&&value==='accepted'){const identity=kind==='toll'?'toll_id':'concept_id';index=rows.findIndex(r=>!r[reportKey]&&source[identity]&&r[identity]===source[identity]);}if(value==='accepted'){const quantity=num(source.quantity)||1,approved={...(index>=0?rows[index]:{}),...source,payer_agent:'customer',collector_agent:source.collector_agent||'company',quantity,unit_amount:amountOf(source)/quantity,customer_payment_method:(kind==='toll'?source.customer_payment_method:reportedExcessPayment(source))||'not_collected',[reportKey]:id};if(index>=0)rows[index]=approved;else rows.push(approved);if(kind==='toll'){const c=w.data.commercial_addons;c.toll_coverage_mode=c.tolls.every(r=>r.payer_agent==='customer')?'customer_roundtrip':'mixed_manual';}}else if(index>=0)rows.splice(index,1);preservePlannedRows();w.dirty=true;w.quote=null;window.OperatorServiceCommercialAddonsV1?.show?.(kind);window.OperatorServiceWorkspaceV2?.sync?.();}renderEmbedded();}
  ...
  function getSaveState(serviceId){if(serviceId!==E.serviceId||!E.detail)return null;return{decisions:Object.fromEntries(E.decisions),note:E.note};}
  function unresolvedEmbedded(){return embeddedRows(E.detail).some(item=>!E.decisions.has(item.kind+':'+item.id))||[...E.decisions.values()].some(d=>d.value==='rejected'&&!d.reason?.trim());}
