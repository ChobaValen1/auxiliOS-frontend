(function(){
  ...
  function comparisonTable(kind,rows,side){
    ...
    return`<div class="os-review-table-wrap"><table class="os-review-table"><thead><tr><th>${head}</th><th>Cantidad</th><th>Monto</th>${method}</tr></thead><tbody>${rows.map(x=>{const name=toll?x.toll_name:x.concept_name,payment=toll?x.customer_payment_method:reportedExcessPayment(x);return`<tr><td>${esc(name||'Sin identificar')}</td><td>${esc(num(x.quantity||1))}</td><td>${money(amountOf(x))}</td>${side==='reported'?`<td>${esc(paymentLabel(payment))}</td>`:''}</tr>`}).join('')}</tbody></table></div>`;
  }
  ...
  function reportedTable(kind,rows,data){
    ...
    return`<div class="os-review-table-wrap"><table class="os-review-table os-review-editable-table"><thead><tr><th>${head}</th><th>Cantidad</th><th>Monto</th><th>Método de pago</th></tr></thead><tbody>${rows.map((x,i)=>toll?tollLine(x,i,data):excessLine(x,i,data)).join('')}</tbody></table></div><div class="os-review-add-wrap"><button type="button" onclick="AuxiliosRemitoReviewV2.addLine('${kind}')" hidden data-review-add>Agregar ${toll?'peaje':'excedente'}</button></div>`;
  }
  ...
  function render(data){
    ...
    $('#os-review-body').innerHTML=`<div data-review-panel="document">${signedDocument(data)}</div><div data-review-panel="review" hidden><section class="os-review-comparison-card"><header class="os-review-comparison-header"><div><span>Control administrativo</span><h3>Planificado vs. informado</h3></div><div class="os-review-matrix-head" aria-hidden="true"><b>Planificado</b><b>Informado</b></div></header>${comparisonSection('toll',data)}${comparisonSection('excess',data)}<div id="os-review-errors" class="os-review-errors"></div></section></div>`;
    const footer=$('#os-review-footer');footer.innerHTML=`<small>${data.can_resolve?'Resolvé Peajes y Excedentes desde sus acciones.':'Documento aprobado o servicio inmutable.'}</small><button class="btn btn-ghost" type="button" onclick="AuxiliosRemitoReviewV2.close()">Cerrar</button>`;tab('document');updateApplyState();void hydrateEvidence();
  }
  ...
  window.AuxiliosRemitoReviewV2={renderInbox,open,retry,close,openEvidence,resolve,chooseAction,applySection,addLine,toggleLineCancel,tab,toggleFullscreen,download};
