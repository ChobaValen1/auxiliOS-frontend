(function(){
  ...
  function comparisonSection(kind,data){
    ...
    return`<section class="os-review-compare-block" data-review-kind="${kind}" data-difference="${difference}"><header><h3>${title}</h3>${reviewActions(kind,difference,data.can_resolve&&canResolve())}</header>${format}<div class="os-review-sides"><article><h4>Planificado</h4>${comparisonTable(kind,plannedRows,'planned')}</article><article><h4>Informado</h4>${comparisonTable(kind,reportedRows,'reported')}</article></div><div class="os-review-resolution" hidden><label>Motivo<textarea data-review-reason placeholder="Indicá brevemente el motivo"></textarea></label></div><div class="os-review-editor" hidden><h4>Modificar lo informado</h4><div class="os-review-edit-table">${editors}</div></div></section>`;
  }
  ...
  function render(data){
    ...
    $('#os-review-body').innerHTML=`<div data-review-panel="document">${signedDocument(data)}</div><div data-review-panel="review" hidden><div class="os-review-intro"><div><span>Control administrativo</span><h3>Planificado vs. informado</h3></div><p>Compará solamente peajes y excedentes. Si hay diferencias, elegí cómo resolverlas.</p></div>${comparisonSection('toll',data)}${comparisonSection('excess',data)}<div id="os-review-errors" class="os-review-errors"></div></div>`;
