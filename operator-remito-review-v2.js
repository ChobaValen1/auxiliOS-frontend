(function(){
  ...
  async function resolve(){
    ...
    try{const {data,error}=await _db.rpc('resolve_operator_service_document_v5',{p_service_id:R.detail.service.service_id,p_action:'approve_and_finalize',p_payload:payload});if(error)throw error;if(data?.document_status!=='approved'||data?.status!=='completed')throw new Error('El cierre no confirmó todos los estados');window.toast?.('Remito aprobado y servicio finalizado','success');close();await window.OperatorServices?.loadServices?.()}
