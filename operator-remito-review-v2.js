(function(){
  ...
  async function open(serviceId){
    ...
    try{const {data,error}=await _db.rpc('get_operator_service_remito_review_v3',{p_service_id:serviceId});if(error)throw error;if(!data?.service||!data?.remito)throw new Error('La respuesta no contiene el remito firmado');R.detail=data;render(data)}
  ...
  const E={detail:null,serviceId:null,hostId:null,decisions:new Map(),busy:false,note:''};
  function embeddedRows(detail){
    ...
    for(const kind of ['toll','excess']){
      ...
      reported.forEach((row,index)=>{
        ...
        const key=comparisonKey(kind,row),matches=plannedKeys.get(key)||0;
  ...
  function renderEmbedded(){
    ...
    host.innerHTML=`<section class="os-embedded-review"><header><b>Diferencias del remito</b><small>Aprobá o rechazá cada cargo informado por el chofer.</small></header><div class="os-embedded-list">${rows.length?rows.map(embeddedLine).join(''):'<div class="os-embedded-ok">✓ Peajes y excedentes coinciden</div>'}</div><div class="os-embedded-note">Al aprobar, se completa el cargo del socio con concepto, importe, medio de pago y peaje.</div><small class="os-embedded-immutable">El remito firmado queda preservado.</small><footer><span>${pending} de ${rows.length} pendientes</span><label class="os-embedded-pending-note"><span>Nota para dejar pendiente</span><textarea id="os-embedded-pending-note" oninput="AuxiliosRemitoReviewV2.noteEmbedded(this.value)" placeholder="Indicá qué falta revisar">${esc(E.note)}</textarea></label><div><button type="button" class="pending" onclick="AuxiliosRemitoReviewV2.leavePending()">Dejar pendiente</button><button type="button" class="finish" ${E.busy?'disabled':''} onclick="AuxiliosRemitoReviewV2.finalizeEmbedded()">${E.busy?'Cerrando…':'Finalizar y cerrar'}</button></div></footer></section>`;
  }
  async function embed(serviceId,hostId='osv4-review-slot'){
    const changed=E.serviceId!==serviceId;E.serviceId=serviceId;E.hostId=hostId;if(changed){E.decisions=new Map();E.note=''}E.busy=false;const host=document.getElementById(hostId);if(!host)return;if(!canResolve()){host.innerHTML='<div class="os-embedded-ok">Documento disponible en modo lectura.</div>';return}host.innerHTML='<div class="os-review-loading">Cargando diferencias…</div>';
    const {data,error}=await _db.rpc('get_operator_service_remito_review_v3',{p_service_id:serviceId});if(error){host.innerHTML=`<div class="os-review-errors visible">${esc(error.message)}</div>`;return}E.detail=data;if(changed){const draft=await _db.rpc('get_operator_service_review_draft_v1',{p_service_id:serviceId});if(!draft.error&&draft.data?.decisions){E.decisions=new Map(Object.entries(draft.data.decisions));E.note=String(draft.data.note||'')}}const keys=new Set(embeddedRows(data).map(item=>`${item.kind}:${item.id}`));for(const key of E.decisions.keys())if(!keys.has(key))E.decisions.delete(key);renderEmbedded();
  }
  ...
  function reasonEmbedded(kind,id,reason){const key=`${kind}:${id}`,current=E.decisions.get(key)||{value:'rejected'};E.decisions.set(key,{...current,reason});const host=document.getElementById(E.hostId),rows=embeddedRows(E.detail),pending=rows.filter(item=>!E.decisions.has(`${item.kind}:${item.id}`)||E.decisions.get(`${item.kind}:${item.id}`).value==='rejected'&&!E.decisions.get(`${item.kind}:${item.id}`).reason?.trim()).length,counter=host?.querySelector('footer>span');if(counter)counter.textContent=`${pending} de ${rows.length} pendientes`}
  ...
  async function finalizeEmbedded(){if(E.busy)return;const rows=embeddedRows(E.detail),unresolved=rows.some(item=>{const d=E.decisions.get(`${item.kind}:${item.id}`);return!d||d.value==='rejected'&&!d.reason?.trim()});if(unresolved){window.alert('No se puede cerrar el servicio. Debés aprobar o rechazar todas las diferencias antes de finalizar.');return}const wizard=window.OperatorServices?.S?.wizard;if(wizard?.dirty){window.toast?.('Guardá los cambios del formulario antes de finalizar. La revisión se actualizará con las diferencias restantes.','info');return}E.busy=true;renderEmbedded();const {data,error}=await _db.rpc('resolve_operator_service_document_v6',{p_service_id:E.serviceId,p_action:'approve_and_finalize',p_payload:embeddedPayload()});if(error){E.busy=false;renderEmbedded();window.toast?.(error.message,'error');return}window.toast?.('Servicio finalizado y enviado a Facturación','success');window.cerrarNuevoServicio?.(true);await window.OperatorServices?.loadServices?.()}
  function editEmbedded(){const id=E.serviceId;window.cerrarNuevoServicio?.(true);window.editarServicioOperador?.(id)}
  async function leavePending(){const note=E.note.trim();if(note.length<3)return window.toast?.('Escribí una nota para dejar la revisión pendiente','error');const wizard=window.OperatorServices?.S?.wizard;if(wizard?.dirty){window.toast?.('Guardá los cambios del formulario antes de dejar pendiente.','info');return}const decisions=Object.fromEntries(E.decisions);const {error}=await _db.rpc('save_operator_service_review_draft_v1',{p_service_id:E.serviceId,p_note:note,p_decisions:decisions});if(error)return window.toast?.(error.message,'error');window.cerrarNuevoServicio?.(true);window.toast?.('La revisión quedó pendiente','info')}
  function resetEmbedded(){E.detail=null;E.serviceId=null;E.hostId=null;E.decisions=new Map();E.note='';E.busy=false}

  function tab(){}
  window.AuxiliosRemitoReviewV2={open,retry,close,openEvidence,resolve,chooseGlobalAction,cancelGlobalAction,commitGlobalAction,addLine,toggleLineCancel,embed,decideEmbedded,reasonEmbedded,noteEmbedded,finalizeEmbedded,editEmbedded,leavePending,resetEmbedded,tab};
