-- Zonas v3: las bases viajan al mapa como puntos fijos.
--
-- El mapa mostraba dónde se piden los servicios, pero no desde dónde se
-- atienden. Sin las bases dibujadas, un foco de calor lejos de todo no se
-- distingue de uno pegado a una base: es el mismo manchón naranja.
--
-- El CTE `sede` ya existía para calcular los kilómetros muertos (la distancia
-- entre la base que tomó el servicio y la más cercana). Acá sale también por el
-- payload: mismas reglas, activas y geocodificadas, así que el mapa y el
-- cálculo no pueden discrepar sobre cuáles son las bases.
--
-- El bbox pasa a cubrir servicios Y bases. Si encuadrara sólo sobre los
-- servicios, una base sin trabajo cerca quedaría fuera de cuadro justo cuando
-- es el dato que hay que ver.
...
begin
  ...
  select jsonb_build_object(
    ...
    /* Para encuadrar el mapa sobre los datos en vez de sobre todo el país.
       Cubre servicios Y bases: encuadrando sólo sobre los servicios, una base
       sin trabajo cerca quedaba fuera de cuadro justo cuando es el dato que hay
       que ver. */
    'bbox', (
      ...
      from (
        select lat, lng from puntos
        union all
        select lat, lng from sede
      ) enfoque
    ),
    -- Las bases, para dibujarlas fijas. Mismo CTE que usa el cálculo de
    -- kilómetros muertos: el mapa y el número no pueden discrepar sobre
