-- AuxiliOS · Las transiciones operativas pertenecen exclusivamente al rol Operador.
do $migration$
declare
  v_oid oid;
  v_sql text;
...
begin
  ...
  if v_oid is null then raise exception 'No existe public.transition_operator_service_v2'; end if;
  ...
  if v_sql=v_before then raise exception 'No se encontró la regla de rol esperada en transition_operator_service_v2'; end if;
  execute v_sql;
end;
$migration$;
