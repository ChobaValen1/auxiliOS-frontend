-- Facturación v8: se saca por_base. Era el mismo corte al revés.
--
-- v7 lo había dado vuelta para que por_empresa y por_base fueran las dos
-- direcciones del mismo cubo. En el tablero terminaron leyéndose como lo
-- mismo, así que quedó una sola tabla —prestadora, y adentro sus bases— con
-- las métricas de las dos fusionadas.
--
-- Lo que se pierde: el total de una base sumando todas sus prestadoras. Hoy
-- Piñeyro trabaja para dos y ese número queda partido. Si más adelante hace
-- falta mirar la carga de una base como lugar físico, el corte se repone desde
-- el mismo agrupamiento por (empresa, base) que ya calcula esta función: es
-- plegar para el otro lado, no un dato nuevo.
