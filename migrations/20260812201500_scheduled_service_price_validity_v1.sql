...
DECLARE
  ...
  v_next date;
BEGIN
  IF p_valid_from IS NULL THEN RAISE EXCEPTION 'La vigencia es obligatoria'; END IF;
  IF p_valid_from<=current_date THEN
    RETURN app_private.current_price_card_for_company(p_company_id,true);
  END IF;
  ...
  WHERE r.contract_id=v_contract.contract_id
    ...
    AND (r.valid_until IS NULL OR r.valid_until>=p_valid_from-1)
  ORDER BY r.valid_from DESC,r.version DESC
  LIMIT 1;
  IF v_source.rate_card_id IS NULL THEN
    PERFORM app_private.current_price_card_for_company(p_company_id,true);
    SELECT r.* INTO v_source
    FROM public.company_rate_cards r
    WHERE r.contract_id=v_contract.contract_id
      AND r.status IN ('active','scheduled') AND r.valid_from<p_valid_from
...
DECLARE
  ...
  v_valid_from date:=coalesce(nullif(p_payload->>'valid_from','')::date,current_date);
...
BEGIN
  ...
  IF v_valid_from<current_date THEN RAISE EXCEPTION 'La vigencia no puede comenzar en una fecha pasada'; END IF;
  ...
  v_card:=CASE WHEN v_valid_from>current_date THEN app_private.ensure_scheduled_price_card(v_company,v_valid_from) ELSE app_private.current_price_card_for_company(v_company,true) END;
  ...
  FROM public.company_service_settings css WHERE css.company_id=v_company AND css.concept_id=v_concept;
  ...
  RETURN to_jsonb(v_item)||jsonb_build_object('valid_from',v_valid_from,'scheduled',v_valid_from>current_date);
END
$function$;
...
BEGIN
  ...
  ELSIF v_target_item IS NULL THEN
    INSERT INTO public.company_rate_items(
      rate_card_id,branch_id,billing_base_id,service_code,service_name,base_price,included_km,extra_km_price,
      km_calculation_method,included_wait_minutes,wait_price_per_hour,tolls_mode,tolls_fixed_amount,extraction_fee,
      cancellation_fee,second_unit_fee,minimum_charge,night_surcharge_pct,weekend_surcharge_pct,holiday_surcharge_pct,
      ...
      v_target,branch_id,billing_base_id,service_code,service_name,base_price,included_km,extra_km_price,
      km_calculation_method,included_wait_minutes,wait_price_per_hour,tolls_mode,tolls_fixed_amount,extraction_fee,
      cancellation_fee,second_unit_fee,minimum_charge,night_surcharge_pct,weekend_surcharge_pct,holiday_surcharge_pct,
      is_active,notes,concept_id,can_be_primary,can_be_secondary,pricing_unit,primary_price,secondary_price,code_mode,code_prefix
    FROM public.company_rate_items WHERE rate_item_id=v_source_item.rate_item_id;
  ELSE
...
BEGIN
  ...
  SELECT c.contract_id INTO v_contract FROM public.company_contracts c
  WHERE c.company_id=p_company_id AND c.status='active' AND c.valid_from<=current_date AND (c.valid_until IS NULL OR c.valid_until>=current_date)
  ...
  SELECT s.billing_setting_id INTO v_setting FROM public.company_billing_settings s WHERE s.company_id=p_company_id AND s.is_active ORDER BY s.valid_from DESC,s.updated_at DESC LIMIT 1;
  ...
  RETURN jsonb_build_object(
    ...
    'services',coalesce((SELECT jsonb_agg(jsonb_build_object(
      ...
      'general_price',CASE WHEN v_card IS NULL THEN NULL ELSE (SELECT jsonb_build_object('rate_item_id',i.rate_item_id,'movement_price',CASE WHEN sc.distance_chargeable THEN i.primary_price ELSE NULL END,'km_price',CASE WHEN sc.distance_chargeable THEN i.extra_km_price ELSE NULL END,'unit_price',CASE WHEN sc.distance_chargeable THEN NULL ELSE CASE WHEN sc.service_category='secondary' THEN i.secondary_price ELSE i.primary_price END END,'pricing_unit',i.pricing_unit,'valid_from',(SELECT r.valid_from FROM public.company_rate_cards r WHERE r.rate_card_id=v_card)) FROM public.company_rate_items i WHERE i.rate_card_id=v_card AND i.concept_id=sc.concept_id AND i.is_active AND i.branch_id IS NULL AND i.billing_base_id IS NULL LIMIT 1) END,
      'base_exceptions',CASE WHEN v_card IS NULL THEN '[]'::jsonb ELSE coalesce((SELECT jsonb_agg(jsonb_build_object('rate_item_id',i.rate_item_id,'base_id',b.base_id,'base_name',b.name,'movement_price',CASE WHEN sc.distance_chargeable THEN i.primary_price ELSE NULL END,'km_price',CASE WHEN sc.distance_chargeable THEN i.extra_km_price ELSE NULL END,'unit_price',CASE WHEN sc.distance_chargeable THEN NULL ELSE CASE WHEN sc.service_category='secondary' THEN i.secondary_price ELSE i.primary_price END END,'pricing_unit',i.pricing_unit) ORDER BY b.name) FROM public.company_rate_items i JOIN public.billing_bases b ON b.base_id=i.billing_base_id JOIN public.company_billing_base_links l ON l.base_id=b.base_id AND l.billing_setting_id=v_setting AND l.is_active WHERE i.rate_card_id=v_card AND i.concept_id=sc.concept_id AND i.is_active AND i.branch_id IS NULL AND i.billing_base_id IS NOT NULL),'[]'::jsonb) END,
      'scheduled_prices',CASE WHEN v_contract IS NULL THEN '[]'::jsonb ELSE coalesce((SELECT jsonb_agg(jsonb_build_object(
        'valid_from',r.valid_from,'rate_card_id',r.rate_card_id,'billing_base_id',i.billing_base_id,'base_id',b.base_id,'base_name',b.name,
      ...
      ) ORDER BY r.valid_from,i.billing_base_id NULLS FIRST) FROM public.company_rate_cards r JOIN public.company_rate_items i ON i.rate_card_id=r.rate_card_id AND i.concept_id=sc.concept_id AND i.is_active AND i.branch_id IS NULL LEFT JOIN public.billing_bases b ON b.base_id=i.billing_base_id WHERE r.contract_id=v_contract AND r.status='scheduled' AND r.valid_from>current_date),'[]'::jsonb) END
...
BEGIN
  ...
  RETURN coalesce((SELECT jsonb_agg(jsonb_build_object(
    ...
    'valid_from',rc.valid_from,'scheduled',rc.status='scheduled',
    ...
    'before',jsonb_build_object('movement_price',CASE WHEN sc.distance_chargeable THEN nullif(ae.before_data->>'primary_price','')::numeric ELSE NULL END,'km_price',CASE WHEN sc.distance_chargeable THEN nullif(ae.before_data->>'extra_km_price','')::numeric ELSE NULL END,'unit_price',CASE WHEN sc.distance_chargeable THEN NULL ELSE CASE WHEN sc.service_category='secondary' THEN nullif(ae.before_data->>'secondary_price','')::numeric ELSE nullif(ae.before_data->>'primary_price','')::numeric END END),
    'after',jsonb_build_object('movement_price',CASE WHEN sc.distance_chargeable THEN nullif(ae.after_data->>'primary_price','')::numeric ELSE NULL END,'km_price',CASE WHEN sc.distance_chargeable THEN nullif(ae.after_data->>'extra_km_price','')::numeric ELSE NULL END,'unit_price',CASE WHEN sc.distance_chargeable THEN NULL ELSE CASE WHEN sc.service_category='secondary' THEN nullif(ae.after_data->>'secondary_price','')::numeric ELSE nullif(ae.after_data->>'primary_price','')::numeric END END)
...
CREATE OR REPLACE FUNCTION public.get_operator_service_context_v1(p_company_id uuid,p_scheduled_for timestamp with time zone DEFAULT now())
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  IF v_role NOT IN ('administracion','facturacion','operador','supervision') THEN RAISE EXCEPTION 'Sin permiso para preparar servicios'; END IF;
  IF NOT EXISTS(SELECT 1 FROM public.companies c WHERE c.company_id=p_company_id AND c.status='active') THEN RAISE EXCEPTION 'Prestadora inexistente o inactiva'; END IF;
  SELECT s.* INTO v_setting FROM public.company_billing_settings s WHERE s.company_id=p_company_id AND s.is_active AND s.valid_from<=v_date AND (s.valid_until IS NULL OR s.valid_until>=v_date) ORDER BY s.valid_from DESC,s.updated_at DESC LIMIT 1;
  ...
  ELSE
    SELECT count(*) INTO v_base_count FROM public.company_billing_base_links l JOIN public.billing_bases b ON b.base_id=l.base_id AND b.is_active WHERE l.billing_setting_id=v_setting.billing_setting_id AND l.is_active;
    IF v_base_count=0 THEN v_blocking:=v_blocking||jsonb_build_array(jsonb_build_object('code','missing_bases','message','La prestadora no tiene bases habilitadas.')); END IF;
  END IF;
  SELECT count(*) INTO v_service_count FROM public.company_service_settings css JOIN public.service_concepts sc ON sc.concept_id=css.concept_id WHERE css.company_id=p_company_id AND css.is_enabled AND sc.is_active AND sc.billing_family<>'system';
  IF v_service_count=0 THEN v_blocking:=v_blocking||jsonb_build_array(jsonb_build_object('code','missing_services','message','La prestadora no tiene servicios habilitados.')); END IF;

  v_card:=app_private.price_card_for_company_date(p_company_id,v_date);
  SELECT count(*) INTO v_missing_price_count FROM public.company_service_settings css JOIN public.service_concepts sc ON sc.concept_id=css.concept_id WHERE css.company_id=p_company_id AND css.is_enabled AND sc.is_active AND sc.billing_family<>'system' AND (v_card IS NULL OR NOT EXISTS(SELECT 1 FROM public.company_rate_items i WHERE i.rate_card_id=v_card AND i.concept_id=sc.concept_id AND i.is_active AND i.branch_id IS NULL AND i.billing_base_id IS NULL));
  IF v_missing_price_count>0 THEN v_warnings:=v_warnings||jsonb_build_array(jsonb_build_object('code','services_without_price','message',format('%s servicio(s) habilitado(s) no tienen precio configurado para la fecha del servicio.',v_missing_price_count),'count',v_missing_price_count)); END IF;

  RETURN jsonb_build_object(
    'company',(SELECT jsonb_build_object('company_id',c.company_id,'name',coalesce(c.trade_name,c.legal_name)) FROM public.companies c WHERE c.company_id=p_company_id),
    'ready',jsonb_array_length(v_blocking)=0,'blocking_issues',v_blocking,'warnings',v_warnings,
    'billing',CASE WHEN v_setting.billing_setting_id IS NULL THEN NULL ELSE jsonb_build_object('billing_setting_id',v_setting.billing_setting_id,'route_mode',v_setting.route_mode,'toll_calculation_mode',v_setting.toll_calculation_mode,'covered_radius_km',v_setting.covered_radius_km,'movement_charge_until_km',v_setting.movement_charge_until_km,'requires_verified_base',v_setting.requires_verified_base) END,
    'bases',CASE WHEN v_setting.billing_setting_id IS NULL THEN '[]'::jsonb ELSE coalesce((SELECT jsonb_agg(jsonb_build_object('base_id',b.base_id,'name',b.name,'address',b.address,'latitude',b.latitude,'longitude',b.longitude,'google_place_id',b.google_place_id,'address_verified',b.address_verified,'route_ready',NOT v_setting.requires_verified_base OR (b.address_verified AND b.latitude IS NOT NULL AND b.longitude IS NOT NULL)) ORDER BY b.name) FROM public.company_billing_base_links l JOIN public.billing_bases b ON b.base_id=l.base_id AND b.is_active WHERE l.billing_setting_id=v_setting.billing_setting_id AND l.is_active),'[]'::jsonb) END,
    'services',coalesce((SELECT jsonb_agg(jsonb_build_object('concept_id',sc.concept_id,'name',sc.name,'category',sc.service_category,'pricing_unit',sc.default_pricing_unit,'distance_chargeable',sc.distance_chargeable,'code_mode',coalesce(css.code_mode,'fixed'),'requires_own_code',coalesce(css.requires_own_code,false) OR coalesce(css.code_mode,'fixed')='manual','has_price',v_card IS NOT NULL AND EXISTS(SELECT 1 FROM public.company_rate_items i WHERE i.rate_card_id=v_card AND i.concept_id=sc.concept_id AND i.is_active AND i.branch_id IS NULL AND i.billing_base_id IS NULL),'available',v_card IS NOT NULL AND EXISTS(SELECT 1 FROM public.company_rate_items i WHERE i.rate_card_id=v_card AND i.concept_id=sc.concept_id AND i.is_active AND i.branch_id IS NULL AND i.billing_base_id IS NULL)) ORDER BY sc.sort_order,sc.name) FROM public.company_service_settings css JOIN public.service_concepts sc ON sc.concept_id=css.concept_id WHERE css.company_id=p_company_id AND css.is_enabled AND sc.is_active AND sc.billing_family<>'system'),'[]'::jsonb)
  );
END
$function$;

-- Los motores existentes se actualizan in-place: scheduled es un estado efectivo por fecha,
-- no una nueva versión de API.
DO $patch$
DECLARE
  v_oid oid;
  v_def text;
BEGIN
  SELECT p.oid INTO v_oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='app_private' AND p.proname='calculate_operator_service_quote_v4_full' AND p.prokind='f' LIMIT 1;
  IF v_oid IS NOT NULL THEN
    v_def:=pg_get_functiondef(v_oid);
    v_def:=replace(v_def,"r.status='active'","r.status IN ('active','scheduled')");
    v_def:=replace(v_def,'ORDER BY r.version DESC,r.valid_from DESC LIMIT 1','ORDER BY r.valid_from DESC,r.version DESC LIMIT 1');
    EXECUTE v_def;
  END IF;

  SELECT p.oid INTO v_oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='get_operator_category_tariff_v3' AND p.prokind='f' LIMIT 1;
  IF v_oid IS NOT NULL THEN
    v_def:=pg_get_functiondef(v_oid);
    v_def:=replace(v_def,"r.status='active'","r.status IN ('active','scheduled')");
    v_def:=replace(v_def,'ORDER BY r.version DESC,r.valid_from DESC LIMIT 1','ORDER BY r.valid_from DESC,r.version DESC LIMIT 1');
  ...
  SELECT p.oid INTO v_oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='get_operator_service_catalog' AND p.prokind='f' LIMIT 1;
  IF v_oid IS NOT NULL THEN
    v_def:=pg_get_functiondef(v_oid);
    v_def:=replace(v_def,"r.status='active'","r.status IN ('active','scheduled')");
    v_def:=replace(v_def,'order by r.version desc,r.valid_from desc limit 1','order by r.valid_from desc,r.version desc limit 1');
  ...
  SELECT p.oid INTO v_oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='get_company_tariffs_v4' AND p.prokind='f' LIMIT 1;
  IF v_oid IS NOT NULL THEN
    v_def:=pg_get_functiondef(v_oid);
    v_def:=replace(v_def,"WHERE r.contract_id=v_contract.contract_id AND r.status='active' ORDER BY r.version DESC LIMIT 1","WHERE r.contract_id=v_contract.contract_id AND r.status IN ('active','scheduled') AND r.valid_from<=current_date AND (r.valid_until IS NULL OR r.valid_until>=current_date) ORDER BY r.valid_from DESC,r.version DESC LIMIT 1");
    EXECUTE v_def;
  END IF;
END
$patch$;
