-- AuxiliOS · ACTIVADO del Chofer con motivos cerrados v3.
-- El flujo ya no solicita detalle libre; cada motivo persiste un codigo estable
-- y un texto canonico para historial/operaciones.
...
begin
  ...
  v_reason_label := case v_reason_code
    when 'absent_or_not_towable' then 'Cancelado por socio ausente / vehículo no se puede remolcar'
    when 'delay' then 'Cancelado por Demora'
    when 'client' then 'Cancelado por Cliente'
    when 'us' then 'Cancelado por nosotros'
    when 'created_without_assignment' then 'Creación sin Asignación'
    else 'Otro'
  end;
