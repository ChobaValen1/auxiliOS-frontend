-- Facturación v5: se saca el margen. No hay con qué calcularlo.
--
-- v3 y v4 publicaban un "margen" = (km facturados - km reales) / km reales, y
-- lo mismo por base. La premisa era que los tramos Base→Origen y Destino→Base
-- son kilómetros que la flota recorre sin cobrar. Es falsa: la flota NO sale de
-- la base. Esos dos tramos son la fórmula de cobro acordada con la empresa,
-- no un recorrido.
--
-- Entonces esa razón no mide margen ni mide vacío: mide el coeficiente del
-- método de facturación, y presentada como rendimiento miente. Con 4 servicios
-- reales daba 240,9%, que alguien podía leer como que la operación rinde el
-- triple.
--
-- Los kilómetros que la flota recorre de verdad están en el odómetro de las
-- jornadas (daily_logs), no en los servicios. Cruzar servicio contra jornada es
-- lo que haría honesta a esa razón, y hoy no se puede: el módulo de servicios
-- recién arranca. Mejor no mostrar el dato que mostrarlo mal.
--
-- Qué se saca del payload: reales.margen, reales.margen_anterior,
-- reales.km_facturados, y por cada base km_real, km_comparable y
-- servicios_con_dato. Queda km_reales con su desglose de medidos y calculados,
-- que son números crudos y verificables. El resto de la función no cambia.
