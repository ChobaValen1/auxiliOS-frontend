-- Operaciones v4: que las filas de totales cierren.
--
-- Las tablas por camión y por chofer ahora cierran con una fila de totales, y
-- esa fila dejó a la vista dos cosas que antes no se podían explicar:
--
--   * Los servicios no dan lo mismo en las tres lecturas. 622 en el período,
--     620 en la tabla por camión (dos remitos sin jornada, así que no cuelgan
--     de ningún móvil) y 621 en la tabla por chofer (un remito de alguien que
--     no cerró ninguna jornada en el rango). Los tres números están bien; lo
--     que faltaba era decir por qué difieren, así que `descartes` suma
--     `servicios_sin_chofer` al lado del `servicios_sin_camion` que ya estaba.
--
--   * H/jornada del pie no daba igual que la razón de arriba: 12,2 contra
--     12,72. Cada fila divide por las jornadas con horario utilizable, pero el
--     pie sólo tenía la columna `jornadas`, que incluye las seis sin hora de
--     cierre. Ahora cada fila de `por_chofer` viaja con `jornadas_con_horas`
--     aunque no se muestre como columna: es el denominador con el que tiene que
--     cerrar, el mismo que usa cada fila.
...
begin
  ...
  select jsonb_build_object(
    ...
    'descartes', jsonb_build_object(
      ...
      -- Un remito de un chofer que no cerró ninguna jornada en el rango no
      -- aparece en la tabla por chofer. Sin este número, el total de esa tabla
      -- no cierra contra el de la tabla por camión y no hay forma de saber por qué.
      'servicios_sin_chofer', (
        select count(*)::int from servicio s
        where s.driver_id is null
           or not exists (select 1 from jornada j where j.driver_id = s.driver_id)
      )
    ),
    ...
    'por_chofer', (
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'jornadas',          x.jornadas,
        -- Viaja aunque no se muestre como columna: es el denominador con el que
        -- la tabla cierra H/jornada, el mismo que usa cada fila.
        'jornadas_con_horas', x.jornadas_con_horas,
        'servicios',         x.servicios,
        'horas_por_jornada',
...
comment on function public.dashboard_operaciones_v1(date, date, int[], uuid[]) is
  'Métricas agregadas de Operaciones: servicios, kilómetros, jornadas y combustible. El bloque combustible desglosa el gasto por medio de pago (la app cuando el método es app), con ticket promedio, precio por litro, km/litro, litros cada 100 km y costo por km. Cada fila de por_chofer trae jornadas_con_horas para que la tabla cierre H/jornada con el mismo denominador que usa cada fila, y descartes informa los servicios que no se pueden colgar de un camión o de un chofer. Granularidad diaria hasta 31 días, semanal hasta 120, mensual más allá. Sólo administración y supervisión.';
