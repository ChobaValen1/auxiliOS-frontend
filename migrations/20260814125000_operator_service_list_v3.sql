-- AuxiliOS · mesa operativa definitiva de Servicios
-- Agrega al listado únicamente los datos operativos solicitados y el importe que debe cobrar el socio.
-- No modifica la liquidación ni los importes: customer_amount_due se deriva de peajes a cargo del cliente + todos los excedentes.
    ...
    from (
      select s.scheduled_for,
        to_jsonb(s)||jsonb_build_object(
          ...
          'customer_amount_due',
            coalesce((select sum(ot.total_amount) from public.operator_service_tolls ot where ot.service_id=s.service_id and ot.source='planned' and ot.payer_agent='customer'),0)
            + coalesce((select sum(oe.total_amount) from public.operator_service_excess_charges oe where oe.service_id=s.service_id),0)
        ) row_data
