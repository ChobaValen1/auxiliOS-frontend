-- Facturación v6: por_empresa trae sus bases adentro, y vuelven los km reales.
--
-- El donut de "Servicios por empresa" se reemplaza por una tabla que se abre:
-- cada prestadora muestra sus bases debajo. Para eso el payload deja de tener
-- por_empresa y por_base como dos listas sueltas que el front no puede cruzar
-- (por_base agrega TODAS las empresas juntas) y pasa a anidar.
--
-- Vuelven los km reales por grupo, que v5 había sacado. La razón por la que
-- salieron sigue en pie —la flota no sale de la base, así que los tramos
-- Base→Origen y Destino→Base son la fórmula de cobro y no un recorrido— pero
-- acá el número no va solo: en la misma fila se ven los km facturados Y los km
-- reales, o sea las dos entradas de la cuenta. Un porcentaje solo, de titular,
-- se lee como rendimiento; el mismo porcentaje al lado de sus dos operandos se
-- lee como lo que es, y sirve para comparar métodos de cobro entre prestadoras.
--
-- km_comp es la pieza que hace honesta esa cuenta: son los km FACTURADOS de los
-- mismos servicios que tienen km real. Dividir el total facturado por los km
-- reales de un subconjunto daría un margen inventado. Con cobertura completa
-- km_comp y km coinciden; con cobertura parcial, no, y manda km_comp.
