-- Facturación v4: los km de Origen a Destino salen de la ruta ya guardada.
--
-- El operador crea el servicio y Google calcula la ruta. Esa ruta se guardaba
-- entera en route_legs, tramo por tramo, y nadie la estaba mirando:
--
--   base_origin_destination_base → 3 tramos: Base→Origen, ORIGEN→DESTINO, Destino→Base
--   origin_destination           → 1 tramo:  ORIGEN→DESTINO
--   base_origin                  → 1 tramo:  Base→Origen (no hay Origen→Destino)
--
-- El modo no se persiste en el servicio (sale de los parámetros de facturación
-- de la empresa), así que se deduce de la cantidad de tramos. Con un solo tramo
-- hay ambigüedad, y se resuelve mirando si el servicio tiene destino cargado:
-- sin destino, ese tramo sólo puede ser Base→Origen.
--
-- Esto se verificó contra datos reales: en el servicio 302243 los tramos dan
-- 60.798 + 72.996 + 33.925 m, y el chofer informó 73 km en el remito. El tramo
-- del medio ES lo que el chofer reporta. O sea que km_reales y el tramo
-- Origen→Destino miden lo mismo, y por eso uno puede cubrir al otro.
--
-- Qué cambia en el payload: km_reales pasa a ser el informado por el chofer y,
-- cuando no lo informó, el calculado de la ruta. Se informa cuántos vienen de
-- cada fuente: un número medido y uno estimado no valen igual, y el que mira el
-- tablero tiene derecho a saber cuál está viendo.
