-- AuxiliOS · Facturación · corrección de zona horaria en total de Peajes.
  ...
  return jsonb_build_object(
    'rows',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'scheduled_for',s.scheduled_for,'completed_at',s.completed_at,'service_billing_status',s.billing_status,
        'company_id',s.company_id,'company_name',coalesce(c.trade_name,c.legal_name,'Prestadora'),
        'billing_base_name',coalesce(b.name,'Sin base'),'origin',s.origin,'destination',s.destination,
        'customer_name',s.customer_name,'vehicle_plate',s.vehicle_plate,'toll_name',coalesce(t.toll_name_snapshot,'Peaje'),
        'road',t.road_snapshot,'direction',t.direction_snapshot,'quantity',coalesce(t.quantity,1),
        'amount',coalesce(t.total_amount,0),'currency',coalesce(t.currency,s.currency,'ARS'),
        'source',t.source,'payment_method',t.payment_method,'crossed_at',t.crossed_at,'payer_agent',t.payer_agent
      ...
      where s.status='completed' and s.billing_status in ('pending','reviewed')
        and t.payer_agent='provider' and coalesce(t.total_amount,0)>0
