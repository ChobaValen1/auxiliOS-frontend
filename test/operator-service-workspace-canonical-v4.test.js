...
const workspaceJs = fs.readFileSync('operator-service-workspace-v2.js', 'utf8');
...
test('Nuevo Servicio tiene un único renderer de tres columnas', () => {
...
test('operator-service-wizard queda como controlador y no puede pintar otra UI', () => {
  ...
  assert.doesNotMatch(controller, /operator-service-desktop\.css/);
  assert.doesNotMatch(controller, /Alta operativa/);
});

test('el workspace de tres columnas se carga directamente desde bootstrap', () => {
  const wizard = config.indexOf("'/operator-service-wizard.js'");
  const workspace = config.indexOf("'/operator-service-workspace-v2.js'");
  const review = config.indexOf("'/operator-service-workspace-review-v3.js'");
  assert.ok(wizard >= 0);
  assert.ok(workspace > wizard);
  assert.ok(review > workspace);
  assert.match(config, /service_workspace_v2\s*=\s*true/);
  assert.match(flags, /flags\.service_workspace_v2=true/);
});

test('la PWA elimina los assets del alta anterior', () => {
