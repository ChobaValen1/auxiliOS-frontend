...
declare
  ...
  v_fuel integer; v_toll uuid; v_excess uuid; v_payload jsonb; v_result jsonb; v_context jsonb; v_created jsonb; v_replay jsonb;
...
begin
  ...
  perform public.save_driver_ad_hoc_remito_v3(v_payload,v_operation);
  if not exists(select 1 from public.remitos r where remito_id=v_remito and r.km_reales=18 and r.firma_imagen_url='https://example.invalid/qa-signature.png' and r.firmado_at is not null and (to_jsonb(r)->>'conformidad_servicio')::boolean and (to_jsonb(r)->>'conformidad_cargos')::boolean and (to_jsonb(r)->>'sin_danos')::boolean) then raise exception 'Signature, confirmations or kilometers were lost'; end if;
  ...
  update public.users set role_id=(select role_id from public.roles where name='administracion') where user_id=v_operator;
  insert into public.fuel_records(truck_id,log_id,fuel_date,liters,price_per_liter,payment_method) values(v_truck,v_log,current_date,20,1000,'efectivo') returning fuel_id into v_fuel;
  perform public.update_fuel_record(v_fuel,'{"liters":25,"price_per_liter":1200}'::jsonb,'QA correction validation');
  if not exists(select 1 from public.fuel_records where fuel_id=v_fuel and liters=25 and price_per_liter=1200 and total_cost=30000 and correction_reason='QA correction validation') then raise exception 'Fuel correction failed'; end if;
end;
$test$;
rollback;
select 'PASS: remito handoff, signature, confirmations and fuel correction; all synthetic rows rolled back' as result;
